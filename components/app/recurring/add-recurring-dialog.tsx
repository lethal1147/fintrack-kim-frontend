        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-1">
          ...
          <div className="border-border flex overflow-hidden rounded-lg border text-sm">
            {(["expense", "income"] as RecurringKind[]).map((k) => (
              ...
              >
                {k === "expense" ? t("kindExpense") : t("kindIncome")}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="rec-amount">{t("amountLabel")}</Label>
              <Input
                id="rec-amount"
                type="number"
                min="0.01"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="rec-due">{t("nextDueLabel")}</Label>
            {showMonthHelper && (
              <div className="mb-1.5 flex gap-1.5">
                <button
                  type="button"
                  onClick={() => setNextDue(firstOfNextMonth())}
                  className="border-border bg-background text-muted-foreground hover:bg-muted hover:text-foreground rounded-md border px-2.5 py-1 text-xs transition-colors"
                >
                  {t("helperFirstOfMonth")}
                </button>
                <button
                  type="button"
                  onClick={() => setNextDue(endOfNextMonth())}
                  className="border-border bg-background text-muted-foreground hover:bg-muted hover:text-foreground rounded-md border px-2.5 py-1 text-xs transition-colors"
                >
                  {t("helperEndOfMonth")}
                </button>
